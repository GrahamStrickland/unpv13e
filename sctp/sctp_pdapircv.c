...
uint8_t *pdapi_recvmsg(int sock_fd, int *rdlen, SA *from, int *from_len,
  ...
  while ((*msg_flags & MSG_EOR) == 0) {
    ...
    if (left < SCTP_PDAPI_NEED_MORE_THRESHOLD) {
      sctp_pdapi_readbuf =
          realloc(sctp_pdapi_readbuf, sctp_pdapi_rdbuf_sz + SCTP_PDAPI_INCR_SZ);
      if (sctp_pdapi_readbuf == NULL) {
    ...
    rdsz = Sctp_recvmsg(sock_fd, &sctp_pdapi_readbuf[at_in_buf], left, NULL,
                        &frmlen, NULL, msg_flags);
    at_in_buf += rdsz;
  }
  *rdlen = at_in_buf;
