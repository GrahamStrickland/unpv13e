...
int net_open(char *path, int oflags, void *addr, int addrlen) {
  int fd;
  int flags;
...
int net_bind(int fd, void *addr, int addrlen) {
  struct {
...
int net_send(int fd, char *buf, int len, char *to_addr, int addrlen) {
  struct strbuf ctlbuf;
...
int net_recv(int fd, char *buf, int len, char *from_addr, int addrlen) {
  struct strbuf ctlbuf;
  struct strbuf databuf;
  struct {
  ...
  c = (char *)&netdata;
  bcopy(&(c[netdata.unitdata_ind.SRC_offset]), from_addr, addrlen);
