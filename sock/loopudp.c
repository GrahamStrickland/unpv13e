  ...
  for (;;) {
    ...
    if (FD_ISSET(STDIN_FILENO, &rset)) { /* data to read on stdin */
      ...
      if (crlf) {
        ...
        if (connectudp) {
          ...
          if (sendto(sockfd, wbuf, ntowrite, 0, (struct sockaddr *)&servaddr,
        ...
        if (connectudp) {
          ...
          if (sendto(sockfd, rbuf, nread, 0, (struct sockaddr *)&servaddr,
    ...
    if (FD_ISSET(sockfd, &rset)) { /* data to read from socket */
      if (server) {
        ...
        nread = recvfrom(sockfd, rbuf, readlen, 0, (struct sockaddr *)&cliaddr,
                         &clilen);
      ...
      oncemore:
        ...
        nread = recvfrom(sockfd, rbuf, readlen, 0, (struct sockaddr *)&servaddr,
                         &servlen);
        if (nread < 0)
