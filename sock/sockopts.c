...
void sockopts(int sockfd, int doall) {
  ...
  if (dontroute) {
    option = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_DONTROUTE, &option, sizeof(option)) <
        0)
      err_sys("SO_DONTROUTE setsockopt error");

    option = 0;
    optlen = sizeof(option);
    if (getsockopt(sockfd, SOL_SOCKET, SO_DONTROUTE, &option, &optlen) < 0)
  ...
  if (broadcast) {
    option = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_BROADCAST, &option, sizeof(option)) <
        0)
    ...
    if (onesbcast) {
      ...
      option = 0;
      optlen = sizeof(option);
      if (getsockopt(sockfd, IPPROTO_IP, IP_ONESBCAST, &option, &optlen) < 0)
  ...
  if (joinip[0]) {
    ...
    if (setsockopt(sockfd, IPPROTO_IP, IP_ADD_MEMBERSHIP, &join, sizeof(join)) <
        0)
  ...
  if (mcastttl) {
    ...
    optlen = sizeof(ttl);
    if (getsockopt(sockfd, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, &optlen) < 0)
  ...
  if (keepalive && doall && udp == 0) {
    option = 1;
    if (setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, &option, sizeof(option)) <
        0)
  ...
  if (nodelay && doall && udp == 0) {
    option = 1;
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY, &option, sizeof(option)) <
        0)
  ...
  if (doall && sndtimeo) {
    ...
    if (setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, &timer, sizeof(timer)) < 0)
