...
void Mcast_join(int sockfd, const SA *grp, socklen_t grplen, const char *ifname,
                u_int ifindex) {
...
int mcast_join_source_group(int sockfd, const SA *src, socklen_t srclen,
  ...
  case AF_INET: {
    ...
    memcpy(&mreq.imr_multiaddr, &((struct sockaddr_in *)grp)->sin_addr,
           sizeof(struct in_addr));
    memcpy(&mreq.imr_sourceaddr, &((struct sockaddr_in *)src)->sin_addr,
           sizeof(struct in_addr));
    ...
    return (setsockopt(sockfd, IPPROTO_IP, IP_ADD_SOURCE_MEMBERSHIP, &mreq,
                       sizeof(mreq)));
  }
#endif
...
void Mcast_join_source_group(int sockfd, const SA *src, socklen_t srclen,
  ...
  if (mcast_join_source_group(sockfd, src, srclen, grp, grplen, ifname,
                              ifindex) < 0)
...
int mcast_block_source(int sockfd, const SA *src, socklen_t srclen,
  ...
  case AF_INET: {
    ...
    return (
        setsockopt(sockfd, IPPROTO_IP, IP_BLOCK_SOURCE, &mreq, sizeof(mreq)));
  }
#endif
