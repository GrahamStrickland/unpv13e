...
static int ga_aistruct(struct addrinfo ***, const struct addrinfo *, void *,
                       int);
...
#define error(e)                                                               \
  ...
  if (hostname != NULL && hostname[0] != '\0' &&
    ...
    aihead->ai_canonname =
        strdup(hptr->h_name != NULL ? hptr->h_name : search[0].host);
...
static int ga_echeck(const char *hostname, const char *servname,
  ...
  case AF_INET:
    if (hintsp->ai_socktype != 0 &&
        (hintsp->ai_socktype != SOCK_STREAM &&
         hintsp->ai_socktype != SOCK_DGRAM && hintsp->ai_socktype != SOCK_RAW))
...
static int ga_nsearch(const char *hostname, const struct addrinfo *hintsp,
...
static int ga_aistruct(struct addrinfo ***paipnext,
                       const struct addrinfo *hintsp, void *addr, int family) {
  struct addrinfo *ai;
...
static int ga_serv(struct addrinfo *aihead, const struct addrinfo *hintsp,
                   const char *serv) {
  int port, rc, nfound;
  struct servent *sptr;
...
static int ga_unix(const char *path, struct addrinfo *hintsp,
